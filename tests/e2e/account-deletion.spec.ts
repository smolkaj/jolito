...
test('failed deletion preserves account and deck through reload and retry', async ({
  page,
}) => {
  ...
  await page.route('https://mock.supabase.co/**', async (route) => {
    ...
    if (request.url().includes('/rpc/delete_user_account')) {
      ...
      if (deletionAttempts <= 2) {
        await route.fulfill({
          status: deletionAttempts === 1 ? 404 : 503,
  ...
  for (const failure of [404, 503]) {
    ...
    await page.screenshot({
      path: `test-results/account-deletion-${failure}.png`,
      fullPage: true,
    })
    await page.reload()
  }
