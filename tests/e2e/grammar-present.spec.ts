...
for (const width of [320, 393, 768, 1024, 1280]) {
  test(`present tense remains coherent through setup, recall, completion and offline reload at ${width}px`, async ({
    page,
    context,
  }) => {
    await page.setViewportSize({ width, height: width > 600 ? 900 : 852 })
    await page.goto('/#/grammar')
    await page.getByRole('combobox', { name: 'Tense' }).selectOption('present')
    await expect(
    ...
    await page.screenshot({
      path: `test-results/present-${width}-setup.png`,
      fullPage: true,
    })
    ...
    await expect(page.locator('.expected-row')).toContainText('hablo')
    const rule = page.locator('.expected-row .diff-rule')
    await expect(rule).toContainText('Rule: Replace -ar with -o.')
    ...
    expect((await auditAccessibility(page)).violations).toEqual([])
    await page.screenshot({
      path: `test-results/present-${width}-correction.png`,
      fullPage: true,
    })
    ...
    await page.screenshot({
      path: `test-results/present-${width}-complete.png`,
      fullPage: true,
    })
    ...
    await expect(page.getByRole('heading', { level: 1 })).not.toContainText(
      'Todos los días yo … con la familia por teléfono.',
    )
    ...
    expect(
      saved.cards.filter((card) => card.grammar?.topic === 'present'),
    ).toHaveLength(8)
    expect(
