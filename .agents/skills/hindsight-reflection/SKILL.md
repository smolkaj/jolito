...
Before submitting a PR for review, execute this protocol, post the evaluation in dialogue, and yield the turn.
...
Bias toward rebuilding cleanly: the first pass of non-trivial work is almost always an exploratory draft. Because newly written tests act as the safety ratchet, rebuilding from scratch takes only a fraction of the time and produces a radically simpler, more durable result.

1. **Rebuild cleanly:** (Expected for non-trivial exploratory work). The draft exhibits exploratory accretion, journey sediment, or structural divergence. Outline the greenfield plan, open the PR, post your recommendation, launch the review loop in parallel, and yield the turn. Upon user confirmation, keep the newly written tests (the safety ratchet), revert the draft implementation files (`git restore <files>`), and implement the clean greenfield design in one direct pass.
...
3. **Proceed:** High-bar exception. Choose this only if the draft already matches the greenfield design with zero compensatory layering or journey sediment. Open the PR, post your recommendation, launch the review loop in parallel, and yield the turn.
