...
2. Review the whole change according to its real risks. Consider correctness, security, data compatibility, failure behavior, needless complexity, duplication, dead code, obsolete paths, misleading or redundant comments, architecture and cognitive cost (especially hidden state, ambient magic, or divergent mechanisms), test and documentation sufficiency, and—when user-facing—accessibility and rendered behavior. Ask: _In hindsight, would we do it again? Was this actually an improvement?_ If the change reflects exploratory accretion and starting fresh would yield a materially simpler design, treat that as a blocking finding. Treat material opportunities to make the code clearer and easier to evolve as blockers, even when behavior is correct. Do not manufacture findings to fill categories or block on unrelated pre-existing debt.
