...
export async function parseAnkiDeck(
  ...
  filename?: string,
  now = Date.now(),
  packageParser?: AnkiPackageParser,
): Promise<ParseAnkiResult> {
  ...
  if (typeof fileData !== 'string') {
    ...
    if (bytes.length >= 2 && bytes[0] === 0x50 && bytes[1] === 0x4b) {
      if (packageParser) {
        return packageParser(bytes, now)
      }
      return {
        success: false,
        error: 'Binary .apkg archive parsing requires an Anki package parser.',
      }
    }
    ...
    try {
      ...
      return parseAnkiDeck(decodedText, filename, now, packageParser)
    } catch {
      return {
        success: false,
