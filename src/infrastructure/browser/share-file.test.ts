...
describe('shareFileNative', () => {
  it('calls nativePlugin.shareFile and returns true on completed', async () => {
  ...
  it('returns false when dismissed/cancelled', async () => {
    const shareFileMock = vi
      .fn<NativeShareFilePlugin['shareFile']>()
  ...
  it('catches exceptions and returns false safely', async () => {
    const shareFileMock = vi
      .fn<NativeShareFilePlugin['shareFile']>()
