...
export function buildMnemonicPrompt(spanish: string, english: string): string {
  return `Create a short, memorable sound-alike mnemonic hook to remember that the Mexican Spanish phrase "${spanish}" means "${english}". Find an English word or phrase that sounds like "${spanish}", then create a vivid, quirky 1-sentence mental image connecting that sound to "${english}". Example: "en voz alta" (out loud) sounds like "voice alter" -> Picture altering your voice so everyone hears you out loud. Output ONLY the 1-sentence mnemonic hook on a single line without any preamble or conversational filler.`
}
