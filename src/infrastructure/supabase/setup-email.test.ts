...
describe('Email Routing Managed Senders', () => {
  it('returns canonical sender, signin, and login aliases for a given domain', () => {
    const senders = getManagedSenders('joli.to')
    expect(senders).toEqual(['a@joli.to', 'signin@joli.to', 'login@joli.to'])
  })
  ...
  it('handles subdomain environments cleanly', () => {
    ...
    expect(senders).toEqual([
      'a@preview.joli.to',
      'signin@preview.joli.to',
      'login@preview.joli.to',
    ])
  })
})
