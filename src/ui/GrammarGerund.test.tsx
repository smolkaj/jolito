...
it('keeps a gerundio draft through tense selection and interruptions, then grades the full phrase', async () => {
  ...
  expect(screen.getByText('Spanish present progressive')).toBeVisible()
  await user.click(screen.getByRole('button', { name: 'Start practice' }))
  await user.type(screen.getByRole('textbox'), 'estoy habla')
  ...
  expect(screen.getByRole('textbox')).toHaveValue('estoy habla')
  await user.type(screen.getByRole('textbox'), 'ndo')
