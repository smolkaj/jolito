...
11. **Principled audio contracts & hardware mute switch discipline.** Spoken audio and earcons follow strict platform contracts across web and native shells ([ADR 0006](adr/0006-neural-speech-synthesis-and-voice-licensing.md)).
    - _Auto-play zero network delay:_ Auto-play during card review or answer reveal (`explicit: false`) must never block on network requests. If cached in memory, play neural audio immediately (0ms); if uncached, failover immediately (0ms) to the native operating system speech engine without waiting.
    - _Explicit request fidelity:_ Explicit user taps on speaker buttons (`explicit: true`) must honor user intent. If cached, play immediately (0ms); if uncached and online, fetch cloud neural audio with a bounded 1.0s timeout ceiling, falling back to native OS speech on timeout or failure.
    - _Hardware silent switch separation:_ Ambient playback (auto-play and sensory earcons) runs under category `ambient` (`AVAudioSession.Category.ambient` / `navigator.audioSession.type = 'ambient'`) and is muted when devices are in silent mode. Explicit speaker taps switch to category `playback` for the duration of playback, ensuring explicit requests are audible even when the phone is on silent, and restore `ambient` upon completion.
